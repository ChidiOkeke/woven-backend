...
const payForJob = async (req, res) => {
    ...
    try{
        ...
        const contractor = await Profile.findOne(({ where: { id: job.contractor_id } }));
        ...
        if(userProfile.id !== client.id){
            return res.status(httpStatus.BAD_REQUEST).json({
                error: Errors.VALIDATION_FAILS,
            });
        }
       
        if(+client.balance < +job.amount){
            return res.status(httpStatus.BAD_REQUEST).json({
        ...
        const transaction = await sequelize.transaction(async (t) => {
            ...
            const debitResult = await Profile.update(
                { balance:  +client.balance - +job.amount},
                { where: { id: client.id} },
                { transaction: t }
            );

            const creditResult = await Profile.update(
                { balance:  +contractor.balance + +job.amount},
